...
export default function Materiales() {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("¿Seguro quieres eliminar este material?")) return;
    try {
      await fetch(`http://localhost:3000/api/materiales/${id}`, { method: "DELETE" });
      fetchMateriales();
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const method = formData.id_material ? "PUT" : "POST";
    const url = formData.id_material
      ? `http://localhost:3000/api/materiales/${formData.id_material}`
  ...
  const pagedMateriales = materiales.slice(startIndex, startIndex + pageSize);
  const totalPages = Math.ceil(materiales.length / pageSize);
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)", padding: "2rem", fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
        ...
        <div style={{ background: "white", borderRadius: "16px", padding: "2rem", marginBottom: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h1 style={{ margin: 0, fontSize: "1.875rem", fontWeight: 700, color: "#1e3c72" }}>Materiales</h1>
        ...
        <div style={{ background: "white", borderRadius: "16px", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", overflowX: "auto" }}>
          {loading ? <p>Cargando...</p> : error ? <p>{error}</p> : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {pagedMateriales.map((m) => (
                  <tr key={m.id_material} style={{ borderBottom: "1px solid #e2e8f0" }}>
                    <td style={{ padding: "0.75rem" }}>{m.nombre}</td>
                    <td style={{ padding: "0.75rem" }}>{m.cantidad}</td>
                    <td style={{ padding: "0.75rem" }}>{m.descripcion}</td>
                    <td style={{ padding: "0.75rem", textAlign: "center" }}>{m.activo ? "Sí" : "No"}</td>
                    <td style={{ padding: "0.75rem", display: "flex", gap: "0.5rem", justifyContent: "center" }}>
                      <button onClick={() => handleEdit(m)} style={{ background: "#3b82f6", color: "white", border: "none", padding: "0.4rem 0.6rem", borderRadius: "6px", cursor: "pointer" }}>Editar</button>
                      <button onClick={() => handleDelete(m.id_material)} style={{ background: "#ef4444", color: "white", border: "none", padding: "0.4rem 0.6rem", borderRadius: "6px", cursor: "pointer" }}>Eliminar</button>
                    </td>
                  </tr>
                ))}
                {pagedMateriales.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ textAlign: "center", padding: "1rem" }}>No hay materiales registrados</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {totalPages > 0 && (
            <div style={{ marginTop: "1rem", textAlign: "center", color: "#6b7280", fontSize: "0.875rem" }}>
              Mostrando {startIndex + 1} - {Math.min(startIndex + pageSize, materiales.length)} de {materiales.length} materiales
            </div>
          )}
        </div>
        ...
        {formVisible && (
          <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
            <div style={{ background: "white", borderRadius: "16px", padding: "2rem", minWidth: "320px", boxShadow: "0 20px 40px rgba(0,0,0,0.1)", display: "flex", flexDirection: "column", gap: "1rem" }}>
              <h3 style={{ margin: 0, fontSize: "1.5rem", fontWeight: 700 }}>{formData.id_material ? "Editar Material" : "Nuevo Material"}</h3>
              ...
              <input type="number" placeholder="Cantidad" value={formData.cantidad} onChange={(e) => setFormData({ ...formData, cantidad: Number(e.target.value) })} style={{ padding: "0.6rem", borderRadius: "8px", border: "1px solid #cbd5e1" }} />
